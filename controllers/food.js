...
module.exports.search = async (req, res) => {
    ...
    const { centerName, waitTime, price } = req.body;
    ...
    const showStalls = await Stall.find({
        location: hawkerCenter._id,
    }).populate("reviews");
    // console.log(`this show stalls: ${showStalls}`);
    // console.log();
    // const reviews = showStalls.reviews;
    const reviewsArray = [];
    for (let i = 0; i < showStalls.length; i++) {
        // console.log(`reviews: ${showReviews}`);
        const showReviews = showStalls[i].reviews;
        console.log(`reviews: ${showReviews}`);
        const mapPrice = showReviews.map((element, index) => {
            element.price;
        });
        // console.log(`show price ${mapPrice}`);
        // const mapReviews = showReviews.map(() => {});
        // reviewsArray.push(showReviews);
    }
    ...
    // if (showStalls.price && showStalls.waitTime <= waitTime && price) {
    // }

    // const stallIdArray = [];
    // for (let i = 0; i < showStalls.length; i++) {
    //     stallIdArray.push(showStalls[i]._id);
    // }
    // console.log(stallIdArray);

    // const Results = stallIdArray.map(() => {
    //     const showResults = Review.find({
    //         stall: stallIdArray.map(stallIdArray),
    //     });
