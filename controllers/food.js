...
module.exports.recommended = async (req, res) => {
	const newestStalls = await Stall.find({})
		.limit(10)
		.sort({ updatedAt: -1 })
		.populate("author", { name: 1 })
		.populate("img", { url: 1 })
		.populate("location", { centerName: 1 });
	console.log(newestStalls);
	if (!newestStalls) {
		throw new UserError("Stalls not found", 404);
	}
	console.log(newestStalls);
	res.status(200).json({ stalls: newestStalls });
};
...
module.exports.search = async (req, res) => {
	...
	const validStalls = showStalls.filter(
		(stall) => stall.calcPrice <= priceRange && stall.numReviews > 0 && stall.calcWait <= waitTime
	);
